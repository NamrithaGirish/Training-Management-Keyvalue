...
export class MaterialService {
  ...
  constructor(private materialRepository: MaterialRepository,private sessionService:SessionService) {}
  
  async createMaterial(materialDto: CreateMaterialDto): Promise<Material> {
    const material = plainToInstance(Material, instanceToPlain(materialDto));
    material.session= await this.sessionService.findOneById(materialDto.session_id)
  ...
  async updateMaterial(id: number, materialDto: UpdateMaterialDto): Promise<Material> {
          ...
          const materialData = plainToInstance(Material, instanceToPlain(materialDto));
          materialData.session=await this.sessionService.findOneById(materialDto.session_id)
