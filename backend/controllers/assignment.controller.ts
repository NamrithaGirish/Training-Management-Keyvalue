...
export default class AssignmentController {
  ...
  async deleteAssignment(req: Request, res: Response, next: NextFunction) {
    try {
      const assignmentId = parseInt(req.params.id, 10);
      const deletedAssignment = await this.assignmentService.deleteAssignment(
        assignmentId
      );
  ...
  async submitAssignment(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const assignmentId = parseInt(req.params.id, 10);
      const submissionData = plainToInstance(AssignmentSubmissionDto, req.body);
  ...
  async getAssignmentSubmissions(
    ...
    try {
      const assignmentId = parseInt(req.params.id, 10);
      const submissions =
        await this.assignmentService.getSubmissionsByAssignmentId(assignmentId);
