...
export class FeedbackController {
		...
		try {
			...
			if (!feedbacks || feedbacks.length === 0) {
				throw new HTTPException(
					404,
					"No feedback was sent by the user"
				);
			}
			res.status(200).json(feedbacks);
		} catch (error) {
	...
	async getFeedbackByReceiverId(
		...
		try {
			...
			const feedbacks =
				await this.feedbackService.getFeedbackByReceiverId(userId);
			if (!feedbacks || feedbacks.length === 0) {
				throw new HTTPException(
					404,
					"No feedback was rceeived by this user"
				);
			}
			res.status(200).json(feedbacks);
		} catch (error) {
	...
	async getFeedbackBySessionAndType(
		...
		try {
			...
			const feedbacks =
				await this.feedbackService.getFeedbackBySessionAndType(
					type,
					sessionId
				);
			if (!feedbacks || feedbacks.length === 0) {
